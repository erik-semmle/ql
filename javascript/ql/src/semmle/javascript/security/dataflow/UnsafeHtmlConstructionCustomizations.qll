...
module UnsafeHtmlConstruction {
  ...
  /**
   * Gets a reference to `sink` tracked by `t`.
   */
  ...
  class XMLParsedSink extends XssSink {
    ...
    XMLParsedSink() {
      ...
      isUsedInXssSink(xssSink) = [parser.getAResult(), parser.flow()] // <- just assuming that the result of the invocation is relevant.
    }
