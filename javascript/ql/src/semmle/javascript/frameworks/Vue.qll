...
module Vue {
  ...
  class SingleFileComponent extends Instance, MkSingleFileComponent {
    ...
    pragma[noinline]
    private predicate getAnES2015ExportsCandidate(Module m, AbstractValue value) {
      m = getModule() and
      exists(DataFlow::AnalyzedNode node, ExportDeclaration export |
        export.getEnclosingModule() = m
      |
        node = export.(BulkReExportDeclaration).getSourceNode("default")
        or
        node.asExpr() = export.(ExportDefaultDeclaration).getOperand() and
        value = node.getAValue()
      )
    }
    ...
    override DataFlow::Node getOwnOption(string name) {
      ...
      |
        // ES2015 exports
        getAnES2015ExportsCandidate(m, abstractOptions)
        or
        // Node.js exports
