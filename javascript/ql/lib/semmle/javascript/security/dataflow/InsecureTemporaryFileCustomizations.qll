...
module InsecureTemporaryFile {
  ...
  /** A call that opens a file without specifying a secure `mode`. Seen as a sink for insecure temporary file creation. */
  class InsecureFileOpen extends Sink instanceof OpenFileCall {
    InsecureFileOpen() {
      not exists(super.getMode())
      or
      exists(int mode | mode = super.getMode().getIntValue() | not isSecureMode(mode))
    }
  }
