  ...
  class Credentials extends CredentialsNode {
    ...
    Credentials() {
      exists(API::Node callee, string prop |
        ...
        this = callee.getParameter(0).getMember(prop).getARhs() and
        (
...
private module Postgres {
  ...
  class Credentials extends CredentialsNode {
    ...
    Credentials() {
      exists(string prop |
        this = [newClient(), newPool()].getParameter(0).getMember(prop).getARhs()
        or
        this = pgPromise().getParameter(0).getMember(prop).getARhs()
      |
        prop = "user" and kind = "user name"
        or
        prop = "password" and kind = prop
